...
const DEFAULT_COMPANY_SETTINGS: CompanySettings = {
  name: 'Fisiocale',
  slogan: 'Prevenção e Tratamento de Dor',
  logoUrl: '',
};
...
const removeUserId = <T extends Record<string, any>>(item: T): Omit<T, 'user_id'> => {
  const { user_id: _userId, ...cleanItem } = item;
  return cleanItem;
};

const cleanRowsForLocalStorage = <T extends Record<string, any>>(rows: T[]) => {
  return rows.map((row) => removeUserId(row));
};
...
const pullTable = async <T extends Record<string, any>>(
  ...
  if (error) {
    ...
    return safeParse<T[]>(localStorageKey, []);
  }
...
const seedTableIfEmpty = async <T extends { id: string }>(
  table: string,
  userId: string,
  localStorageKey: string,
  defaultRows: T[]
  ...
  const localRows = safeParse<T[]>(localStorageKey, []);
  const sourceRows = localRows.length > 0 ? localRows : defaultRows;
  ...
  const rowsWithUserId = sourceRows.map((row) => ({
    ...row,
  ...
  const { data: seededRows, error: seedError } = await supabase
    .from(table)
    .upsert(rowsWithUserId, { onConflict: 'id' })
    .select('*');

  if (seedError) {
    console.error(`Erro ao criar dados iniciais em ${table}:`, seedError.message, rowsWithUserId);
    localStorage.setItem(localStorageKey, JSON.stringify(sourceRows));
    return sourceRows;
  }

  const cleanSeededRows = cleanRowsForLocalStorage((seededRows ?? rowsWithUserId) as Record<string, any>[]) as T[];
  localStorage.setItem(localStorageKey, JSON.stringify(cleanSeededRows));

  return cleanSeededRows;
};
...
const syncCompanySettings = async (userId: string) => {
  ...
  const localSettings = safeParse<CompanySettings>('companySettings', DEFAULT_COMPANY_SETTINGS);
  const settingsToSeed: CompanySettings = {
    name: localSettings.name || DEFAULT_COMPANY_SETTINGS.name,
    slogan: localSettings.slogan || DEFAULT_COMPANY_SETTINGS.slogan,
  ...
  if (seedError) {
    ...
    localStorage.setItem('companySettings', JSON.stringify(settingsToSeed));
    return;
  }
